...
test('UI: vocab hub tampil dan pilihan ganda bisa dimainkan', async ({ page }) => {
  ...
  // sekarang masuk layar setup dulu (pilih kategori + jumlah)
  await expect(page.getByText('Atur sesi Pilihan Ganda')).toBeVisible()
  await expect(page.getByText('Kategori kosakata')).toBeVisible()
  await page.getByRole('button', { name: /Mulai.*soal/ }).click()
