...
const loadEX = () => {
  try {
    try { localStorage.removeItem('kd-exam-state') } catch {}
...
const acakPilihan = (base) => base.pilihan.map((p, pi) => ({ text: p, correct: Array.isArray(base.jawaban) ? base.jawaban.includes(pi) : pi === base.jawaban })).sort(() => Math.random() - 0.5)
...
export default function ExamMode({ progress, onExit }) {
  ...
  const [bsAns, setBsAns] = useState(() => _s?.bsAns || [])
  const [mcmaSel, setMcmaSel] = useState(() => _s?.mcmaSel || [])
  ...
  const resetJawaban = (q) => {
    setPicked(null); setSusunAns([]); setMcmaSel([])
  ...
  const advance = (ns, pt, nr) => {
    if (pos >= queue.length - 1) { finish(ns, pt, nr); return }
    const nq = queue[pos + 1]
    setPos((p) => p + 1); resetJawaban(nq)
  }

  const scheduleAdvance = (ns, pt, nr) => {
    if (timerRef.current) clearTimeout(timerRef.current)
    timerRef.current = setTimeout(() => advance(ns, pt, nr), 450)
  }
  ...
  const setBS = (i, v) => {
    if (picked !== null) return
    setBsAns((a) => a.map((x, j) => (j === i ? v : x)))
  }
  const submitBS = () => {
    if (picked !== null || !current || bsAns.some((v) => v === null)) return
    const correct = current.pernyataanAcak.every((st, i) => (bsAns[i] === true) === st.benar)
    setPicked(true)
    const ns = { correct: stats.correct + (correct ? 1 : 0), xp: stats.xp }
    const pt = { ...perTipe, bs: { c: (perTipe.bs?.c || 0) + (correct ? 1 : 0), t: (perTipe.bs?.t || 0) + 1 } }
    const nr = [...results, { id: current.id, correct, pickedText: current.pernyataanAcak.map((st, i) => `${st.text}(${bsAns[i] ? 'Benar' : 'Salah'})`).join(' / ') }]
    setStats(ns); setPerTipe(pt); setResults(nr)
    try { progress?.recordAnswer?.(correct) } catch {}
    try { progress?.addXp?.(correct ? 4 : 1) } catch {}
    playClick()
    scheduleAdvance(ns, pt, nr)
  }
  ...
  const toggleMC = (i) => {
    if (picked !== null) return
    setMcmaSel((s) => (s.includes(i) ? s.filter((x) => x !== i) : [...s, i]))
  }
  const submitMCMA = () => {
    if (picked !== null || !current || mcmaSel.length === 0) return
    const correct = mcmaSel.every((i) => current.pilihanAcak[i].correct) && mcmaSel.length === current.pilihanAcak.filter((o) => o.correct).length
    setPicked(true)
    const ns = { correct: stats.correct + (correct ? 1 : 0), xp: stats.xp }
    const pt = { ...perTipe, mcma: { c: (perTipe.mcma?.c || 0) + (correct ? 1 : 0), t: (perTipe.mcma?.t || 0) + 1 } }
    const nr = [...results, { id: current.id, correct, pickedText: mcmaSel.map((i) => current.pilihanAcak[i].text).join(' / ') }]
    setStats(ns); setPerTipe(pt); setResults(nr)
    try { progress?.recordAnswer?.(correct) } catch {}
    try { progress?.addXp?.(correct ? 4 : 1) } catch {}
    playClick()
  ...
  return (
    <div className="mx-auto max-w-xl space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-xs text-slate-600 dark:text-zinc-400">Soal {qNum}/{total}</span>
      </div>
      ...
      <Card className="p-6 sm:p-8 space-y-4 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700">
        ...
        {current.tipe === 'bs' ? (
          <div className="space-y-2">
            {current.pernyataanAcak.map((st, i) => (
              <div key={i} className="rounded-xl border border-slate-200 dark:border-zinc-600 p-3">
                <div className="text-sm text-slate-900 dark:text-white">{st.text}</div>
                <div className="mt-2 flex gap-2">
                  {[true, false].map((v) => (
                    <button key={String(v)} onClick={() => setBS(i, v)} disabled={answered} className={`flex-1 rounded-lg border px-3 py-1.5 text-xs font-bold transition ${bsAns[i] === v ? 'border-slate-900 bg-slate-900 text-white dark:border-white dark:bg-white dark:text-zinc-900' : 'border-slate-200 dark:border-zinc-600 text-slate-600 dark:text-zinc-300'}`}>{v ? 'Benar' : 'Salah'}</button>
                  ))}
                </div>
              </div>
            ))}
            {!answered && <Button onClick={submitBS} disabled={bsAns.some((v) => v === null)} className="w-full">Jawab</Button>}
            {answered && <p className="text-center text-xs text-slate-500">Jawaban tersimpan, lanjut ke soal berikut…</p>}
          </div>
        ) : current.tipe === 'mcma' ? (
          <div className="space-y-2">
            <div className="grid gap-2">
              {current.pilihanAcak.map((o, i) => (
                <button key={i} onClick={() => toggleMC(i)} disabled={answered} className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm font-medium transition ${mcmaSel.includes(i) ? 'border-slate-900 bg-slate-100 dark:border-white dark:bg-zinc-600' : 'border-slate-200 dark:border-zinc-600 bg-white dark:bg-zinc-700 hover:border-slate-400'}`}>
                  <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-md border text-xs ${mcmaSel.includes(i) ? 'border-slate-900 bg-slate-900 text-white dark:border-white dark:bg-white dark:text-zinc-900' : 'border-slate-300 dark:border-zinc-500'}`}>{mcmaSel.includes(i) ? '✓' : ''}</span>
                  {o.text}
                </button>
              ))}
            </div>
            {!answered && <Button onClick={submitMCMA} disabled={mcmaSel.length === 0} className="w-full">Jawab</Button>}
            {answered && <p className="text-center text-xs text-slate-500">Jawaban tersimpan, lanjut ke soal berikut…</p>}
          </div>
        ) : current.tipe === 'susun' ? (
          ...
          <div className="grid gap-2">
            {current.pilihanAcak.map((o, i) => {
              ...
              return (
                <button key={i} onClick={() => answerPG(i)} disabled={answered} className={`rounded-xl border px-4 py-3 text-left text-sm font-medium transition ${isPicked ? 'border-slate-900 bg-slate-100 dark:border-white dark:bg-zinc-600' : 'border-slate-200 dark:border-zinc-600 bg-white dark:bg-zinc-700 hover:border-slate-400'}`}>
                  {o.text}
                </button>
              )
            })}
