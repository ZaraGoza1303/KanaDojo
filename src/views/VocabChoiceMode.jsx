...
const loadVC=()=>{
  try{
    ...
    return { queue, pos: j.pos||0, wrong: (j.wrongIds||[]).map(byId).filter(Boolean), picked: j.picked||null, stats: j.stats||{total:0,correct:0,xp:0}, done: !!j.done, uniqCorrect: j.uniqCorrect||[], uniqWrong: j.uniqWrong||[], initTotal: j.initTotal||30, bestCombo: j.bestCombo||0 }
  }catch{}
  return null
}
...
export default function VocabChoiceMode({ progress, onExit }){
  ...
  const pick = useCallback((arti)=>{
    ...
    setPicked(arti)
    if(correct){ uniqCorrectRef.current.add(current.id); comboRef.current+=1; bestComboRef.current=Math.max(bestComboRef.current, comboRef.current) }
